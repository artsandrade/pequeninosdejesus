      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.resposta == 'inserido') {
            texto_resposta.innerHTML = "Depoimento inserido com sucesso!";
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.resposta == 'alterado') {
          texto_resposta.innerHTML = "Depoimento alterado com sucesso!";
          $('#modal-resposta').modal({
            show: true
          });
...
function remover(id_depoimento) {
  $.ajaxSetup({
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    }
  ...
  $.ajax({
    url: "/painel/depoimentos/remover",
    method: 'post',
    data: {
      '_token': _token,
    ...
    success: function (response) {
      ...
      if (response.resposta == 'removido') {
        window.location.href = "/painel/depoimentos";
      }
    },
    error: function () {
      texto_resposta.innerHTML = "Desculpe, mas tivemos um erro durante essa solicitação. Entre em contato com o suporte ou tente novamente mais tarde!";
...
function modalRemover(nome, id_depoimento) {
  var modal_texto = document.getElementById('modal-texto');
  var modal_botao = document.getElementById('modal-botao');
  
  modal_texto.innerHTML = 'Você tem certeza que deseja remover o depoimento de <b>' + nome + '</b> ?';
  modal_botao.setAttribute('onclick', 'remover(\'' + id_depoimento + '\')');
  $('#modal-center').modal({
    show: true
  });
}